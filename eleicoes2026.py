...
def election_settings(year):
    """Return candidate names and the daily calendar, capped at today for 2026."""
    ...
    end        = pd.Timestamp(ELECTIONS[year]['date'])
    if year == 2026:
        end = min(end,pd.Timestamp.today().normalize())
    dates      = pd.date_range(start=f'{year}-09-01',end=end)
...
def plot_poll_trackers(nacional,national,estadual,from_states,state_polls,candidate=None,year=2022,show=True,national_common=None):
    ...
    show : bool
        Display figures.
    national_common : pd.DataFrame, optional
        National tracker using only institutes shared with state polling.
    ...
    ax.plot(from_states['data'],100*from_states[candidate],color=COLORS[1],label='State aggregate')
    if national_common is not None:
        ax.plot(national_common['data'],100*national_common[candidate],color=COLORS[2],linestyle='--',label='National tracker (shared institutes)')
...
def decompose_state_changes(completed,weights,candidate=None,year=2026,show=True):
    """Decompose daily national tracker changes into weighted state changes.

    Parameters
    ----------
    completed : dict of pd.DataFrame
        Filled state trackers used in the national aggregate, including ZZ.
    weights : pd.Series
        Fixed voting weights indexed by state; normalized internally.
    candidate : str, optional
        Candidate to plot; defaults to the PT candidate.
    year : int
        Election calendar.
    show : bool
        Display the signed stacked-bar chart.

    Returns
    -------
    contributions : pd.DataFrame
        Date, SP, MG, RJ, BA, PR, RS, PE, CE, PA, Others and National change in pp.
        Includes days with any state change, even when changes cancel nationally.
        Others includes overseas votes. The first day has no preceding estimate.
    fig : matplotlib.figure.Figure
        State contributions and net national change on each included date.
    """
    candidates, dates = election_settings(year)
    candidate = candidate or candidates[0]
    ...
    changes   = 100*shares.diff().mul(weights,axis=1)
    selected  = ['SP','MG','RJ','BA','PR','RS','PE','CE','PA']
    other     = [uf for uf in GEOGRAPHIES if uf not in selected]
    changed   = changes.notna().all(axis=1) & changes.abs().gt(1e-12).any(axis=1)
    contributions = changes.loc[changed,selected].copy()

    contributions['Others'] = changes.loc[changed,other].sum(axis=1,min_count=len(other))
    columns = contributions.columns.tolist()
    contributions['National change'] = 100*national.set_index('data')[candidate].diff()
    if not contributions[columns].sum(axis=1).sub(contributions['National change']).abs().le(1e-10).all():
        raise ValueError('State contributions do not reconcile with the national tracker change.')

    fig,ax   = plt.subplots(figsize=(12,6))
    colors   = [STATE_COLORS[state] for state in selected[:5]]+['#173F66','#4D8974','#D78058','#8B4259','#AAB4BE']
    positive = pd.Series(0.0,index=contributions.index)
    negative = positive.copy()
    positions = list(range(len(contributions)))
    for state,color in zip(columns,colors):
        values = contributions[state]
        bottom = positive.where(values>=0,negative)
        ax.bar(positions,values,bottom=bottom,color=color,label=state,width=0.8)
        positive += values.clip(lower=0)
        negative += values.clip(upper=0)
    ax.plot(positions,contributions['National change'],color='#202B33',marker='o',linestyle='none',markersize=5,label='National change')
    ax.axhline(0,color=COLORS[0],linewidth=0.8)
    ax.set_xticks(positions,contributions.index.strftime('%d %b'),rotation=60,ha='right')
    ax.set_ylabel(f'{candidate} change contribution (pp)')
    ax.set_xlabel('Date of state tracker change')
    ax.legend(loc=0,ncol=3)
    if contributions.empty:
        ax.text(0.5,0.5,'No daily state tracker changes',transform=ax.transAxes,ha='center')
    fig.tight_layout()
    if show:
        plt.show()
...
if __name__ == '__main__':
    ...
    state_changes, state_change_figure = decompose_state_changes(completed_previous,weights,year=year,show=False)
    ...
    tracker_figures = plot_poll_trackers(
        nacional    = nacional,
        national    = national,
        estadual    = state_trackers,
        from_states = states_previous,
        state_polls = estadual,
        year        = year,
        show        = False,
        national_common = national_common
    )

    # Composition does not require the current election result, so it is available
    # for both completed elections and 2026.
    institute_summary = composition.rename(columns={
        'national':'National surveys',
        'state':'State surveys',
        'national_pct':'National share (%)',
        'state_pct':'State share (%)'
    })

    if has_results:
        # Result-based evaluation is only available for completed elections.
        results = import_election_results(year=year)
        state_errors, state_error_figure = decompose_state_errors(
            completed_previous,results,weights,year=year,show=False
        )
        institute_accuracy = pollster_accuracy(nacional,estadual,results,year=year)
        accuracy = institute_accuracy.pivot(
            index='Institute',columns='Level',values='Average absolute error (pp)'
        )
        ...
        institute_summary = institute_summary[[
            'National surveys','National share (%)','National error (pp)',
            'State surveys','State share (%)','State error (pp)'
        ]].rename_axis('Institute')

        print('\nInstitute composition and accuracy '
              '(tracking period; errors are average absolute errors with equal survey weights):')
        print(institute_summary.to_string(float_format=lambda x:f'{x:.2f}',na_rep='—'))
    else:
        institute_summary = institute_summary[[
            'National surveys','National share (%)',
            'State surveys','State share (%)'
        ]].rename_axis('Institute')
        print('\nInstitute composition (tracking period; election results not yet available):')
        print(institute_summary.to_string(float_format=lambda x:f'{x:.2f}',na_rep='—'))
    ...
    if common and has_results:
        ...
        print('\nAll selected vs shared institutes '
              '(institute frequencies and geographic coverage can still differ):')
